    ...
    for v_file in video_files:
        ...
        with open(v_file, 'r') as f:
            ...
            for v in videos:
                ...
                filmstrip_filename = os.path.join(result_dir_path, v['id'], '{}_filmstrip.jpg'.format(v['id']))
                try:
                    response = session.get(v['thumbnail_url'], headers=HEADERS)
                    if not response.ok:
                ...
                try:
                    response = session.get(v['filmstrip_60_url'], headers=HEADERS)
                    if not response.ok:
                        logger.error('Can not download filmstrip from video {}'.format(v['filmstrip_60_url']))
                        download_failed += 1
                        failed_count += 1
                    else:
                        with file(filmstrip_filename, 'wb') as img_file:
                            img_file.write(response.content)
                        download_successful += 1
                        successful_count += 1
                except Exception as e:
                    logger.error('Can not download filmstrip from video {}: {}'.format(v['filmstrip_60_url'], str(e)))
                    download_failed += 1
                    failed_count += 1
                    if os.path.exists(filmstrip_filename):
                        os.remove(filmstrip_filename)
