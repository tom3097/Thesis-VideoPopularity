...
for (vid_file in video_files)
{
    ...
    {
        video_date <- substr(video_info$snippet$publishedAt, 1, nchar('yyyy-mm'))
...
channel_likes <- as.numeric(channel_likes)
points <- (views_count+1) / channel_likes
norm_points <- log2(points)

wd = getwd()
setwd(file.path(r_path, 'results'))
png(filename = paste('yt_', gsub(',', '_', args$date), '_', total_videos, '_views.png', sep = ''))
