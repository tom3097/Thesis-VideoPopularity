...
for (vid_file in video_files)
{
    ...
    {
        video_date <- substr(video_info$created_time_readable, 1, nchar('yyyy-mm'))
        if (any(dates == video_date))
        {
            views_count <- c(views_count, video_info$views_total)
            user_likes <- c(user_likes, video_info$user_likes)
            total_videos <- total_videos + 1
        }
    }
}

points <- (views_count+1) / user_likes
norm_points <- log2(points)

wd = getwd()
setwd(file.path(r_path, 'results'))
png(filename = paste('dm_', gsub(',', '_', args$date), '_', total_videos, '_views.png', sep = ''))
