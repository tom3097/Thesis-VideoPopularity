...
class FacebookCrawler {
    ...
    /**
     * Path to a file where metadata from pages will be saved.
     *
     ...
     * @brief Constructor.
     *
     * @param array $facebook_data        Array with facebook client informations: app_id, app_secret, token.
     ...
     */
    public function __construct($facebook_data, $page_file, $video_file, $log_dir) {
        ...
        $longLivedAccessToken = $oAuth2Client->getLongLivedAccessToken($facebook_data['token']);
    ...
    /**
     * @brief Performs filtering. Checks whether the video meets the conditions.
     *
     * @param array $video_data           The array with video's metadata.
     *
     * @return boolean                    TRUE if video meets the conditions, FALSE otherwise.
     */
    private function perform_filtering($video_data) {
        $date = substr($video_data['created_time']['date'], 0, 7);
        if(!in_array($date, $this->condition_array)) {
            return FALSE;
        }
        $views = $video_data['viewsCount']
        if($views < $this->min_views) {
            return FALSE;
        }
        return TRUE;
    }
    ...
    private function analyze_page($page) {
        $fields = '?fields=id,name,description,about,category,category_list,fan_count';
     ...
     *
     * @param array $page_videos          Array with metadata from videos for currently analyzed page.
     ...
     */
    private function save_videos($page_videos, $video_file, $total_page_videos) {
        $this->logger->info('Saving to file...');
        $this->logger->info('Total page videos: ' . $total_page_videos);
        $j_str = json_encode($page_videos, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES);
    ...
    private function analyze_page_videos($page, $page_info) {
        ...
        while(!is_null($video_response)) {
            ...
            foreach($video_data as $video_d) {
                if($this->perform_filtering($video_d)) {
                    ...
                    try {
                        ...
                        array_push($page_videos, $result_array);
                        $total_page_videos += 1;
                        $this->total_videos += 1;
                    }
                    catch(Exception $e) {
            ...
            if($request_counter == $this->max_requests_per_save) {
                $request_counter = 0;
                $this->save_videos($page_videos, $video_file, $total_page_videos);
            }
            try {
...
$facebook_crawler = new FacebookCrawler($facebook_data, $page_file, $video_file, $log_dir);
$facebook_crawler->add_content_providers($content_file);
