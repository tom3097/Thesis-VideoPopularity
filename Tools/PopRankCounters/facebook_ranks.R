...
for (vid_file in video_files)
{
    ...
    {
        key = substr(video_info$created_time$date, 1, nchar('yyyy-mm'))
        viewCount_numerical = as.numeric(video_info$viewsCount)
        followersCount_numerical = as.numeric(video_info$page_likes)
...
write(json_val, paste('facebook_DEPTH_',DEPTH ,'.json', sep=''))
