...
class DailymotionTrainValGen(object):
    def __init__(self, metadata_dir_path, metadata_file_pattern, ranges_dict_file, images_directory,
        ...
        """Creates dailymotion train val files generator

        Args:
            metadata_dir_path (string): Path to a directory where dailymotion metadata are stored.
            metadata_file_pattern (string): Dailymotion metadata file pattern.
            ranges_dict_file (string): Path to a file with popularity class scores.
            images_directory (string): Path to a directory where dailymotion images are stored.
            image_pattern (string): Dailymotion image file pattern.
        ...
        Path to a directory where dailymotion images are stored.
        """

        self.image_pattern = image_pattern
        """string

        Dailymotion image file pattern.
        """
    ...
    def prepare_class_dictionary(self):
        ...
        for video_f in self.video_files:
            with open(video_f, 'r') as f:
                videos = json.load(f)
                for v in videos:
                    view_count = v['views_total']
                    followers_count = v['user_likes']
                    score = self.count_video_score(view_count, followers_count)
                    date = v['created_time_readable'][:len('yyyy-mm')]
...
if __name__ == '__main__':
    ...
    dailymotion_train_val_gen = DailymotionTrainValGen(metadata_dir_path, metadata_file_pattern, ranges_dict_file, images_directory,image_pattern,
    train_factor, train_output, val_output)
    dailymotion_train_val_gen.start()
